...
import { getProductTypeById } from '#/api/product-settings.ts';
...
interface TypeViewDialogProps {
    open: boolean;
    onOpenChange: (open: boolean) => void;
    productType: IProductType | null;
}

export default function TypeViewDialog({ open, onOpenChange, productType }: TypeViewDialogProps) {
    ...
    const { data: typeDetails, isLoading } = useQuery({
        queryKey: [QUERY_KEY.PRODUCT_SETTINGS.TYPE_DETAILS, productType?.id],
        queryFn: () => getProductTypeById(productType!.id),
        enabled: open && !!productType?.id
    });
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="max-w-xl max-h-[85vh] flex flex-col p-0 overflow-hidden bg-background">
                <DialogHeader className="px-6 pt-6 pb-2">
                    ...
                    <DialogDescription className="text-xs">Audit details and timestamps for this product classification.</DialogDescription>
                </DialogHeader>
                        ...
                        typeDetails && (
                            <>
                                <div className="space-y-2">
                                    <Label className="font-semibold text-foreground/80 flex items-center gap-1.5">Type Name</Label>
                                    <Input disabled value={typeDetails.name} className="h-9 bg-background/50" />
                                </div>
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Textarea
                                        disabled
                                        value={typeDetails.description || 'No description provided.'}
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs text-muted-foreground bg-muted/30 p-3 rounded-lg border border-border/40">
                                        ...
                                        {typeDetails.deletedAt && (
                                            <div className="sm:col-span-2 text-destructive font-medium border-t pt-2 mt-1">
                                                ...
                                                {format(new Date(typeDetails.deletedAt), 'MMMM dd, yyyy - hh:mm a')}
                                            </div>
                                        )}
                                    </div>
                                </div>
