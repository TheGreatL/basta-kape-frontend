    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="max-w-2xl max-h-[85vh] flex flex-col p-0 overflow-hidden bg-background">
                {open && role && <RoleEditDialogContent role={role} onOpenChange={onOpenChange} />}
            </DialogContent>
        </Dialog>
    );
}

interface RoleEditDialogContentProps {
    role: IRoleListItem;
    onOpenChange: (open: boolean) => void;
}

function RoleEditDialogContent({ role, onOpenChange }: RoleEditDialogContentProps) {
    ...
    const updateMutation = useMutation({
        ...
        onSuccess: () => {
            ...
            queryClient.invalidateQueries({ queryKey: [QUERY_KEY.RBAC.ROLE_DETAILS, role.name] });
