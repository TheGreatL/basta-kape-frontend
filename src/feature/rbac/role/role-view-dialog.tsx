    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="max-w-2xl max-h-[85vh] flex flex-col p-0 overflow-hidden bg-background">
                {open && role && <RoleViewDialogContent role={role} onOpenChange={onOpenChange} />}
            </DialogContent>
        </Dialog>
    );
}

interface RoleViewDialogContentProps {
    role: IRoleListItem;
    onOpenChange: (open: boolean) => void;
}

function RoleViewDialogContent({ role, onOpenChange }: RoleViewDialogContentProps) {
                    ...
                    <>
                        ...
                        <div className="space-y-3 pt-2">
                            <div className="flex items-center justify-between border-b pb-2">
                                ...
                                <span className="text-xs text-muted-foreground font-medium">Toggle permissions and adjust access scope limits.</span>
                            </div>
