...
interface OptionRecipeDialogProps {
    open: boolean;
    onOpenChange: (open: boolean) => void;
    option: IModifierOption | null;
}

export default function OptionRecipeDialog({ open, onOpenChange, option }: OptionRecipeDialogProps) {
    ...
    React.useEffect(() => {
        if (recipe) {
            form.reset({
                ...
                ingredients: recipe.ingredients.map((ing: IModifierRecipeIngredient) => ({
    ...
    const createRecipeMutation = useMutation({
        mutationFn: (payload: RecipeFormValues) => createModifierOptionRecipe(option!.id, payload),
        onSuccess: () => {
    ...
    const updateRecipeMutation = useMutation({
        mutationFn: (payload: RecipeFormValues) => updateModifierOptionRecipe(option!.id, payload),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: [QUERY_KEY.PRODUCTS.MODIFIER_OPTION_RECIPE, option?.id] });
            queryClient.invalidateQueries({ queryKey: [QUERY_KEY.INVENTORY.FORECAST] });
    ...
    const deleteRecipeMutation = useMutation({
        mutationFn: () => deleteModifierOptionRecipe(option!.id),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: [QUERY_KEY.PRODUCTS.MODIFIER_OPTION_RECIPE, option?.id] });
            queryClient.invalidateQueries({ queryKey: [QUERY_KEY.INVENTORY.FORECAST] });
    ...
    const restoreRecipeMutation = useMutation({
        mutationFn: () => restoreModifierOptionRecipe(option!.id),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: [QUERY_KEY.PRODUCTS.MODIFIER_OPTION_RECIPE, option?.id] });
            queryClient.invalidateQueries({ queryKey: [QUERY_KEY.INVENTORY.FORECAST] });
                                    ...
                                    <FormField
                                        ...
                                        render={({ field }) => (
                                            <FormItem>
                                                ...
                                                <FormControl>
                                                    <Input
                                                        placeholder="e.g. Standard Oat Milk Customization Recipe"
                                                        {...field}
                                    ...
                                    <FormField
                                        ...
                                        render={({ field }) => (
                                            <FormItem>
                                                ...
                                                <FormControl>
                                                    <Input
                                                        placeholder="e.g. Deducts 220ml of Barista Oat Milk"
                                                        value={field.value || ''}
